...
function App() {
  ...
  function submitWaitlist(e) {
    ...
    const subject = encodeURIComponent('GroundUpp waitlist signup');
    const body = encodeURIComponent(`Please add me to the GroundUpp waitlist.\n\nEmail: ${email}`);
    window.location.href = `mailto:hello@groundupp.co.uk?subject=${subject}&body=${body}`;
    setStatus('Your email app should open now. Send the message to join the waitlist.');
  }
    ...
    <main>
      ...
      <section id="top" className="hero section">
        <div className="heroCopy reveal">
          <div className="eyebrow"><HardHat size={16}/> Built for UK construction businesses</div>
          <h1>Construction management from the ground up.</h1>
          <p className="lead">GroundUpp is a sleek, AI-powered operating system for contractors, groundworkers and site teams who need jobs, labour, materials, equipment and paperwork under control.</p>
          <div className="heroActions">
            <a className="primary" href="#waitlist">Join early access <ArrowRight size={18}/></a>
            <a className="secondary" href="#platform">See what’s coming</a>
          </div>
        ...
        <div className="productShell reveal delay">
          ...
          <div className="alertCard hot">
            <div><strong>Delay risk</strong><p>Drainage run — labour tracking 18% above forecast.</p></div><span>High</span>
          </div>
          ...
          <div className="aiPanel"><Sparkles size={18}/><p>AI suggests moving two workers to Plot 14 tomorrow to protect Friday handover.</p></div>
        </div>
      </section>
      ...
      <section id="platform" className="section split">
        ...
        <p className="sectionText">GroundUpp is being designed for the reality of construction: wet sites, rushed mornings, changing labour, van problems, missing materials and paperwork that still needs to look professional.</p>
      </section>

      <section className="featureGrid section compact">
        {features.map((f) => <article className="feature" key={f.title}><f.icon/><h3>{f.title}</h3><p>{f.text}</p></article>)}
      </section>
      ...
      <section id="workers" className="section workerBlock">
        <div className="phoneMock">
          ...
          <div className="workerList"><span>Log hours</span><span>My jobs</span><span>Report issue</span></div>
        </div>
        <div>
          <p className="eyebrow amber">For the lads on site</p>
          <h2>Powerful in the office. Simple on site.</h2>
          <p className="sectionText">Managers get the detail. Workers get big buttons, clear tasks and fast reporting. Nobody needs to fight through five menus to log a day’s work.</p>
          <div className="moduleList">{modules.map(m => <span key={m}>{m}</span>)}</div>
        </div>
      </section>

      <section className="section proof">
        <div className="proofCard"><ShieldCheck/><h3>Made for serious operators</h3><p>Built for companies that have outgrown WhatsApp, spreadsheets and paper folders — but do not want clunky enterprise software.</p></div>
        <div className="proofCard"><Sparkles/><h3>AI-native from day one</h3><p>Not a chatbot bolted on the side. GroundUpp is planned around proactive summaries, risk flags and document generation.</p></div>
        <div className="proofCard"><HardHat/><h3>Construction-first design</h3><p>Dark, rugged, fast and practical. Premium enough for directors, simple enough for site teams.</p></div>
      </section>

      <section id="waitlist" className="section waitlist">
        <div className="waitCard">
          <p className="eyebrow amber"><Mail size={16}/> Early access</p>
          <h2>Get on the GroundUpp waitlist.</h2>
          <p>Be first to see the demo, test the worker flows and help shape a construction platform built around real site problems.</p>
      ...
      <footer>
        <div className="brand"><span className="mark">GU</span><span>GroundUpp</span></div>
